# PowerShell Script để Build AccSafe Portable (Không cần Admin)
# Script này sẽ tự động xóa và thay thế folder dist-electron

Write-Host "============================================" -ForegroundColor Cyan
Write-Host "  Building AccSafe Portable" -ForegroundColor Cyan
...
# Bước 4: Build Portable executable
...
try {
    ...
    Write-Host "  ✗ ERROR: Failed to build portable executable" -ForegroundColor Red
    Write-Host "  Error: $_" -ForegroundColor Red
    Read-Host "Press Enter to exit"
    exit 1
}
...
if (Test-Path $distElectronPath) {
    $portableExe = Get-ChildItem -Path $distElectronPath -Filter "*portable.exe" -File
    if ($portableExe) {
        ...
        Write-Host "============================================" -ForegroundColor Cyan
        Write-Host ""
        $fileSize = [math]::Round($portableExe.Length / 1MB, 2)
        Write-Host "Output file: dist-electron\$($portableExe.Name)" -ForegroundColor White
        Write-Host "File size: $fileSize MB" -ForegroundColor White
        Write-Host ""
    } else {
        Write-Host "  ⚠ Warning: Portable .exe file not found" -ForegroundColor Yellow
    }
} else {
