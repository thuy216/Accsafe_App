# PowerShell Script để Build AccSafe (Yêu cầu Admin)
# Script này sẽ tự động xóa và thay thế folder dist-electron

Write-Host "============================================" -ForegroundColor Cyan
Write-Host "  Building AccSafe (Requires Admin)" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""
...
if (-not $isAdmin) {
    ...
    Write-Host "  Some build processes may require admin privileges." -ForegroundColor Yellow
    Write-Host ""
...
# Bước 4: Build Windows executable (với code signing disabled)
...
try {
    ...
    Write-Host "  Error: $_" -ForegroundColor Red
    Write-Host ""
    Write-Host "  NOTE: If you see code signing errors, make sure you're running as Administrator" -ForegroundColor Yellow
    Read-Host "Press Enter to exit"
    exit 1
}
